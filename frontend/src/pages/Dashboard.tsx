    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-slate-500 dark:text-slate-400 print:text-slate-700">Real-time overview of satellite operations</p>
        </div>
      ...
      {/* Quick Actions */}
      <section className="rounded-3xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/10 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
      ...
      <section className="rounded-3xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/10 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
        ...
        <div className="space-y-3">
          {missionAlerts.slice(0, 5).map(alert => (
            <div key={alert.id} className={`rounded-2xl border border-slate-200 dark:border-white/10 print:border-slate-300 border-l-4 p-4 ${alertSeverityColors[alert.severity]}`}>
      ...
      <section className="rounded-3xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/10 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search schedule..." value={scheduleSearch} onChange={e => setScheduleSearch(e.target.value)} className="rounded-lg border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/5 py-1.5 pl-9 pr-4 text-sm text-slate-900 dark:text-white print:text-black placeholder-slate-400 dark:placeholder-slate-500 focus:border-sky-500 focus:outline-none" />
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/5">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 print:text-slate-700">Mission</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 print:text-slate-700">Satellite</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 print:text-slate-700">Start</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 print:text-slate-700">End</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 print:text-slate-700">Priority</th>
          ...
          <div className="rounded-3xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/10 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
            ...
            <div className="space-y-3">
              {communicationWindows.slice(0, 2).map(window => (
                <div key={window.id} className="rounded-2xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-slate-50 dark:bg-slate-950/50 print:bg-white p-3 text-sm text-slate-600 dark:text-slate-300 print:text-slate-800">
      ...
      <section className="rounded-3xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/10 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {satelliteHealthSummaries.slice(0, 10).map(sat => (
            <div key={sat.id} className="rounded-2xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-slate-50 dark:bg-slate-950/60 print:bg-white p-3">
              <div className="flex items-center justify-between">
              ...
              <div className="mt-2">
                <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 print:text-slate-700">
            ...
            <div className="space-y-3">
              {groundStations.map(station => (
                <div key={station.id} className="rounded-2xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-slate-50 dark:bg-slate-950/60 print:bg-white p-3">
                  ...
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400 print:text-slate-700">{station.location} • {station.latency}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="rounded-3xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/10 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {weatherImpacts.map(w => (
            <div key={w.station} className="rounded-2xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-slate-50 dark:bg-slate-950/60 print:bg-white p-4">
              <div className="flex items-center gap-2">
              ...
              <p className="mt-2 text-xs text-slate-500 dark:text-slate-400 print:text-slate-700">{w.forecast}</p>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            ...
            {recentActivities.slice(0, 8).map((act, i) => (
              <div key={act.id} className="flex gap-3">
                <div className="flex flex-col items-center">
                  ...
                  {i < 7 && <div className="mt-1 h-full w-px bg-white dark:bg-white/10 print:bg-white" />}
                </div>
                <div className="pb-4">
                  <p className="text-sm font-medium text-slate-900 dark:text-white print:text-black">{act.title}</p>
                  <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400 print:text-slate-700">{act.description}</p>
        ...
        <div className="rounded-3xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/10 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
          ...
          <div className="space-y-3">
            {systemStatus.map(s => (
              <div key={s.name} className="flex items-center justify-between rounded-2xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-slate-50 dark:bg-slate-950/60 print:bg-white p-3">
                <div className="flex items-center gap-3">
