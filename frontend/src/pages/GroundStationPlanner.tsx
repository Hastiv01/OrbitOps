    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold text-white">Ground Station Planner</h1>
          <p className="mt-1 text-slate-400">Manage ground station availability and assignments</p>
        </div>
      ...
      <div className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-glow backdrop-blur-xl">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search stations..." value={search} onChange={e => setSearch(e.target.value)} className="rounded-lg border border-white/10 bg-white/5 py-1.5 pl-9 pr-4 text-sm text-white placeholder-slate-500 focus:border-sky-500 focus:outline-none" />
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-white/5">
              {filteredStations.map(gs => (
                <tr key={gs.id} className={`transition hover:bg-white/5 cursor-pointer ${selectedStation?.id === gs.id ? 'bg-sky-500/10' : ''}`} onClick={() => setSelectedStation(selectedStation?.id === gs.id ? null : gs)}>
                  <td className="px-3 py-3 text-sm font-medium text-white">{gs.name}</td>
                  <td className="px-3 py-3 text-sm text-slate-300">{gs.country}</td>
                  <td className="px-3 py-3">{statusBadge(getStatus(gs))}</td>
                  <td className="px-3 py-3 text-sm text-slate-300">{gs.availability}%</td>
                  <td className="px-3 py-3 text-sm text-slate-300">{gs.latency}ms</td>
                  <td className="px-3 py-3 text-sm text-slate-300">{gs.bandwidth}</td>
                  <td className="px-3 py-3">{weatherBadge(gs.weather)}</td>
                  <td className="px-3 py-3 text-sm text-slate-300">{gs.coverage}%</td>
                  <td className="px-3 py-3 text-sm text-slate-300">{gs.antennas}</td>
      ...
      {selectedStation && (
        <div className="rounded-3xl border border-sky-500/30 bg-white/10 p-5 shadow-glow backdrop-blur-xl">
          <p className="mb-4 text-lg font-semibold text-white">{selectedStation.name} — Details</p>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-3">
              <p className="text-xs text-slate-400">Country</p>
              <p className="mt-1 text-sm font-medium text-white">{selectedStation.country}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-3">
              <p className="text-xs text-slate-400">Frequency</p>
              <p className="mt-1 text-sm font-medium text-white">{selectedStation.frequency}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-3">
              <p className="text-xs text-slate-400">Next Maintenance</p>
              <p className="mt-1 text-sm font-medium text-white">{selectedStation.nextMaintenance}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-3">
              <p className="text-xs text-slate-400">Connected Satellites</p>
              <p className="mt-1 text-sm font-medium text-white">{selectedStation.connectedSats}</p>
            </div>
          </div>
          <div className="mt-4">
            <p className="text-sm text-slate-400 mb-2">Assigned Missions</p>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <div className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-glow backdrop-blur-xl">
          <p className="mb-4 text-lg font-semibold text-white">Station Utilization</p>
          <div className="h-56">
        ...
        <div className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-glow backdrop-blur-xl">
          <p className="mb-4 text-lg font-semibold text-white">Communication Load</p>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-glow backdrop-blur-xl">
          <p className="mb-4 text-lg font-semibold text-white">Availability</p>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
          ...
          <div className="flex justify-center gap-3 mt-2">
            {availabilityPie.map(d => (
              <span key={d.name} className="flex items-center gap-1 text-xs text-slate-400"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: d.color }} />{d.name} ({d.value})</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-glow backdrop-blur-xl">
        <div className="mb-4 flex items-center justify-between">
          <p className="text-lg font-semibold text-white">Communication Schedule</p>
          <div className="relative">
            <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <input type="text" placeholder="Search..." value={queueSearch} onChange={e => setQueueSearch(e.target.value)} className="rounded-lg border border-white/10 bg-white/5 py-1.5 pl-9 pr-4 text-sm text-white placeholder-slate-500 focus:border-sky-500 focus:outline-none" />
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-white/5">
              {filteredQueue.map(q => (
                <tr key={q.id} className="transition hover:bg-white/5">
                  <td className="px-3 py-3 text-xs text-slate-400">{q.id}</td>
                  <td className="px-3 py-3 text-sm text-white">{q.satellite}</td>
                  <td className="px-3 py-3 text-sm text-slate-300">{q.station}</td>
                  <td className="px-3 py-3 text-xs text-slate-300">{new Date(q.scheduledTime).toLocaleString()}</td>
                  <td className="px-3 py-3 text-sm text-slate-300">{q.duration} min</td>
                  ...
                  <td className="px-3 py-3 text-sm text-slate-300">{q.dataSize}</td>
