...
const ConstraintOptimization = () => {
  ...
  const sliderRow = (label: string, key: keyof ConstraintConfig, min: number, max: number, unit: string, step = 1) => (
    <div className="flex items-center gap-4 rounded-2xl border border-slate-200 dark:border-slate-700 print:border-slate-300 bg-slate-50 dark:bg-slate-800/80 print:bg-white px-4 py-3">
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-slate-600 dark:text-slate-400 print:text-slate-700">Configure parameters and optimize mission schedules</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="rounded-3xl border border-slate-200 dark:border-slate-700 print:border-slate-300 bg-white dark:bg-slate-800 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
          <p className="text-sm text-slate-600 dark:text-slate-400 print:text-slate-700">Optimization Score</p>
        ...
        <div className="rounded-3xl border border-slate-200 dark:border-slate-700 print:border-slate-300 bg-white dark:bg-slate-800 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
          <p className="text-sm text-slate-600 dark:text-slate-400 print:text-slate-700">Conflicts</p>
        ...
        <div className="rounded-3xl border border-slate-200 dark:border-slate-700 print:border-slate-300 bg-white dark:bg-slate-800 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
          <p className="text-sm text-slate-600 dark:text-slate-400 print:text-slate-700">Resolved</p>
          <p className="mt-4 text-3xl font-semibold text-emerald-400">{optimized ? suggestedSchedule.filter(s => s.conflictFree).length : '—'}</p>
        </div>
        <div className="rounded-3xl border border-slate-200 dark:border-slate-700 print:border-slate-300 bg-white dark:bg-slate-800 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
          <p className="text-sm text-slate-600 dark:text-slate-400 print:text-slate-700">Remaining</p>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-3xl border border-slate-200 dark:border-slate-700 print:border-slate-300 bg-white dark:bg-slate-800 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
            ...
            {!optimized ? (
              <p className="text-sm text-slate-600 dark:text-slate-400 print:text-slate-700">Run optimization to see results.</p>
            ) : (
              <div className="space-y-3">
                {violations.map((v, i) => (
          ...
          {/* Score Gauge */}
          {optimized && (
            <div className="rounded-3xl border border-slate-200 dark:border-slate-700 print:border-slate-300 bg-white dark:bg-slate-800 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl text-center">
              <p className="text-sm text-slate-600 dark:text-slate-400 print:text-slate-700">Optimization Score</p>
              ...
              <p className="mt-2 text-xs text-slate-600 dark:text-slate-400 print:text-slate-700">{optimizationScore > 80 ? 'Excellent' : optimizationScore > 60 ? 'Good' : 'Needs Improvement'}</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={powerUsageData}>
                ...
                <XAxis dataKey="time" stroke="currentColor" className="text-slate-600 dark:text-slate-400 print:text-black" tick={{ fontSize: 9 }} />
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-white/5 print:divide-slate-300">
                {suggestedSchedule.map(m => (
                  <tr key={m.id} className="transition hover:bg-white dark:bg-slate-800">
                    <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400 print:text-slate-700">{m.optimizedOrder}</td>
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-slate-700 print:border-slate-300 bg-white dark:bg-slate-800 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {optimizationHistory.map(h => (
            <div key={h.id} className="rounded-2xl border border-slate-200 dark:border-slate-700 print:border-slate-300 bg-slate-50 dark:bg-slate-800/80 print:bg-white p-3 text-center">
              <p className="text-xs text-slate-600 dark:text-slate-400 print:text-slate-700">{h.timestamp}</p>
