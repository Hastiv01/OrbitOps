    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-slate-500 dark:text-slate-400 print:text-slate-700">Monitor and forecast resource utilization across the fleet</p>
        </div>
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/10 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
        ...
        <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
          {forecastCharts.map(fc => (
            <div key={fc.title} className="rounded-2xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-slate-50 dark:bg-slate-950/60 print:bg-white p-4">
      ...
      <div className="grid gap-6 xl:grid-cols-[0.4fr_1fr]">
        <div className="rounded-3xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/10 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl flex flex-col items-center justify-center">
          <p className="text-sm text-slate-500 dark:text-slate-400 print:text-slate-700">Overall Risk Level</p>
          ...
          <p className="mt-3 text-xs text-slate-500 dark:text-slate-400 print:text-slate-700">{criticalCount} critical, {highCount} high risk satellites</p>
        </div>
        <div className="rounded-3xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/10 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            {subsystemHealth.map(s => (
              <div key={s.name} className="rounded-2xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-slate-50 dark:bg-slate-950/60 print:bg-white p-3 text-center">
                <p className="text-xs text-slate-500 dark:text-slate-400 print:text-slate-700">{s.name}</p>
                <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-white print:text-black">{s.value}<span className="text-sm text-slate-500 dark:text-slate-400 print:text-slate-700">{s.unit}</span></p>
                <div className="mt-2 flex items-center justify-center gap-2">
                  <Badge variant={s.status === 'Nominal' ? 'success' : s.status === 'Warning' ? 'warning' : 'danger'}>{s.status}</Badge>
                  {s.trend === 'up' ? <FiTrendingUp className="text-emerald-400 text-xs" /> : s.trend === 'down' ? <FiTrendingDown className="text-red-400 text-xs" /> : <FiMinus className="text-slate-500 dark:text-slate-400 print:text-slate-700 text-xs" />}
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/10 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search..." value={resSearch} onChange={e => setResSearch(e.target.value)} className="rounded-lg border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/5 py-1.5 pl-9 pr-4 text-sm text-slate-900 dark:text-white print:text-black placeholder-slate-400 dark:placeholder-slate-500 focus:border-sky-500 focus:outline-none" />
          </div>
        </div>
          ...
          <table className="w-full">
            <thead className="border-b border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/5">
              <tr>
                {[
                  ...
                  <th key={col.key} className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 print:text-slate-700 cursor-pointer hover:text-slate-900 dark:text-white print:text-black transition" onClick={() => toggleResSort(col.key)}>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-white/5 print:divide-slate-300">
              {filteredResources.map(r => (
                <tr key={r.satellite} className="transition hover:bg-white dark:bg-white/5">
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/10 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={p.title} className="rounded-2xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-slate-50 dark:bg-slate-950/60 print:bg-white p-4">
              <div className="flex items-center justify-between">
              ...
              <p className="mt-3 text-sm text-slate-500 dark:text-slate-400 print:text-slate-700">{p.text}</p>
            </div>
          ))}
        </div>
      </div>
