    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-slate-400">Monitor and forecast resource utilization across the fleet</p>
        </div>
      ...
      {/* Resource Forecasts */}
      <div className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-glow backdrop-blur-xl">
        <p className="mb-4 text-lg font-semibold text-white">Resource Forecasts (48h)</p>
        <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
          {forecastCharts.map(fc => (
            <div key={fc.title} className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
              <p className="mb-2 text-sm font-medium text-white">{fc.title}</p>
              <div className="h-40">
      ...
      <div className="grid gap-6 xl:grid-cols-[0.4fr_1fr]">
        <div className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-glow backdrop-blur-xl flex flex-col items-center justify-center">
          ...
          <p className="mt-3 text-xs text-slate-400">{criticalCount} critical, {highCount} high risk satellites</p>
        </div>
        <div className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-glow backdrop-blur-xl">
          <p className="mb-4 text-lg font-semibold text-white">Subsystem Health</p>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            {subsystemHealth.map(s => (
              <div key={s.name} className="rounded-2xl border border-white/10 bg-slate-950/60 p-3 text-center">
                <p className="text-xs text-slate-400">{s.name}</p>
                <p className="mt-2 text-2xl font-bold text-white">{s.value}<span className="text-sm text-slate-400">{s.unit}</span></p>
                <div className="mt-2 flex items-center justify-center gap-2">
                  <Badge variant={s.status === 'Nominal' ? 'success' : s.status === 'Warning' ? 'warning' : 'danger'}>{s.status}</Badge>
                  {s.trend === 'up' ? <FiTrendingUp className="text-emerald-400 text-xs" /> : s.trend === 'down' ? <FiTrendingDown className="text-red-400 text-xs" /> : <FiMinus className="text-slate-400 text-xs" />}
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-glow backdrop-blur-xl">
        <div className="mb-4 flex items-center justify-between">
          <p className="text-lg font-semibold text-white">Resource Utilization by Satellite</p>
          <div className="relative">
            <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <input type="text" placeholder="Search..." value={resSearch} onChange={e => setResSearch(e.target.value)} className="rounded-lg border border-white/10 bg-white/5 py-1.5 pl-9 pr-4 text-sm text-white placeholder-slate-500 focus:border-sky-500 focus:outline-none" />
          </div>
        </div>
          ...
          <table className="w-full">
            <thead className="border-b border-white/10 bg-white/5">
              <tr>
                {[
                  ...
                  <th key={col.key} className="px-4 py-3 text-left text-xs font-semibold text-slate-400 cursor-pointer hover:text-white transition" onClick={() => toggleResSort(col.key)}>
            ...
            <tbody className="divide-y divide-white/5">
              {filteredResources.map(r => (
                <tr key={r.satellite} className="transition hover:bg-white/5">
                  <td className="px-4 py-3 text-sm font-medium text-white">{r.satellite}</td>
      ...
      <div className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-glow backdrop-blur-xl">
        <p className="mb-4 text-lg font-semibold text-white">AI Prediction Explanation</p>
        <div className="space-y-4">
          {[
            ...
            <div key={p.title} className="rounded-2xl border border-white/10 bg-slate-950/60 p-4">
              <div className="flex items-center justify-between">
