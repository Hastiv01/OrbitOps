...
export const satellites: Satellite[] = [
  ...
  ...Array.from({ length: 10 }, (_, i) => {
    ...
    const altitudes = { LEO: 380 + i * 15, MEO: 7500 + i * 200, GEO: 35786 };
    return {
      id: `SAT-${String(num).padStart(3, '0')}`,
      name: `Satellite${names[i]}`,
      status: (i % 7 === 0 ? 'Maintenance' : 'Active') as Satellite['status'],
      orbit,
      altitude: altitudes[orbit],
      inclination: 45 + i * 2,
      batteryHealth: 55 + Math.floor(Math.random() * 40),
      temperature: 35 + Math.floor(Math.random() * 15),
      lastUpdate: new Date().toISOString(),
      groundStations: [`GS-${String((i % 15) + 1).padStart(3, '0')}`, `GS-${String(((i + 3) % 15) + 1).padStart(3, '0')}`],
      currentPayloads: [`PAY-${String((i % 14) + 1).padStart(3, '0')}`],
    };
  }),
];
