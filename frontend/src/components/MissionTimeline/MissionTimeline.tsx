    ...
    <div className="rounded-3xl border border-slate-200 dark:border-slate-700 print:border-slate-300 bg-white dark:bg-slate-800 print:bg-white p-5 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 print:text-slate-700">Live scheduling pulse for the day</p>
        </div>
      </div>
        ...
        {missions.map((mission, index) => (
          <div key={mission.id} className="flex gap-3">
            <div className="flex flex-col items-center">
              ...
              {index < missions.length - 1 ? <div className="mt-1 h-full w-px bg-white dark:bg-slate-800 print:bg-white" /> : null}
            </div>
            <motion.div layout className="flex-1 rounded-2xl border border-slate-200 dark:border-slate-700 print:border-slate-300 bg-slate-50 dark:bg-slate-950/50 print:bg-white p-3">
