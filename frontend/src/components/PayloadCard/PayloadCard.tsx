...
const PayloadCard = ({ payload }: PayloadCardProps) => {
  return (
    <div className="rounded-3xl border border-slate-200 dark:border-white/10 print:border-slate-300 bg-white dark:bg-white/10 print:bg-white p-4 shadow-sm dark:shadow-glow print:shadow-none backdrop-blur-xl">
      <div className="flex items-center justify-between">
        <div>
          <p className="font-semibold text-slate-900 dark:text-white print:text-black">{payload.name}</p>
          <p className="text-sm text-slate-500 dark:text-slate-400 print:text-slate-700">{payload.type}</p>
        </div>
      ...
      <p className="mt-3 text-sm text-slate-500 dark:text-slate-400 print:text-slate-700">Power draw: {payload.power}</p>
    </div>
  );
};
