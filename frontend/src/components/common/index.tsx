    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="flex max-h-[85vh] flex-col rounded-2xl border border-slate-200 bg-white shadow-2xl backdrop-blur-xl dark:border-slate-700 dark:bg-slate-800/95">
              <div className="flex flex-shrink-0 items-center justify-between border-b border-slate-200 px-6 py-4 dark:border-slate-700">
                ...
                <button
                  onClick={onClose}
                  className="rounded-lg bg-slate-100 p-2 text-slate-500 transition hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-white/20"
                >
                  <FiX className="text-lg" />
                </button>
              </div>
...
const variantClasses = {
  ...
  secondary: 'bg-white border-slate-200 text-slate-700 hover:bg-slate-50 dark:bg-slate-800 dark:hover:bg-white/20 dark:text-white border dark:border-white/20',
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="relative">
        ...
        <input
          {...props}
          className={`w-full rounded-lg border bg-white px-4 py-2 text-slate-900 placeholder-slate-400 transition focus:outline-none dark:bg-slate-800 dark:text-white dark:placeholder-slate-500 ${
            icon ? 'pl-10' : ''
          } ${
            error
      ...
      {helperText && <p className="text-xs text-slate-600 dark:text-slate-400">{helperText}</p>}
    </div>
  );
};
...
export const Select: React.FC<SelectProps> = ({ label, error, options, ...props }) => {
  return (
    <div className="space-y-1">
      ...
      <select
        {...props}
        className={`w-full rounded-lg border bg-white px-4 py-2 text-slate-900 transition focus:outline-none dark:bg-slate-800 dark:text-white ${
          error
            ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500/20'
            : 'border-slate-300 focus:border-sky-500 focus:ring-sky-500/20 dark:border-slate-700'
        } focus:ring-1 ${props.className || ''}`}
      >
        <option value="" className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white">Select an option</option>
        {options.map((opt) => (
...
export const TextArea: React.FC<TextAreaProps> = ({ label, error, ...props }) => {
  return (
    <div className="space-y-1">
      ...
      <textarea
        {...props}
        className={`w-full resize-none rounded-lg border bg-white px-4 py-2 text-slate-900 placeholder-slate-400 transition focus:outline-none dark:bg-slate-800 dark:text-white dark:placeholder-slate-500 ${
          error
            ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500/20'
            : 'border-slate-300 focus:border-sky-500 focus:ring-sky-500/20 dark:border-slate-700'
        } focus:ring-1 ${props.className || ''}`}
...
export const Card: React.FC<CardProps> = ({ children, className = '', hover = false }) => {
  return (
    <div
      className={`rounded-2xl border border-slate-200 bg-white p-6 shadow-sm backdrop-blur-xl transition dark:border-slate-700 dark:bg-slate-800 dark:shadow-glow print:border-slate-300 print:shadow-none ${
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="animate-pulse space-y-3 rounded-lg bg-slate-100 p-4 dark:bg-slate-800">
...
export const EmptyState: React.FC<EmptyStateProps> = ({ icon, title, description, action }) => {
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border border-slate-200 bg-white p-12 text-center dark:border-slate-700 dark:bg-slate-800 print:border-slate-300 print:bg-white">
      ...
      {description && <p className="mb-6 text-slate-600 dark:text-slate-400 print:text-black">{description}</p>}
...
export const ProgressBar: React.FC<ProgressBarProps> = ({ value, max = 100, className = '', showLabel = true }) => {
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      <div className="h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800 print:border print:border-slate-300">
...
export const Tabs: React.FC<TabsProps> = ({ tabs, defaultTab }) => {
  ...
  return (
    <div>
      <div className="flex gap-2 border-b border-slate-200 dark:border-slate-700 print:hidden">
        {tabs.map((tab) => (
          <button
            key={tab.id}
  ...
  return (
    <Card>
      <div className="flex items-start justify-between">
        <div>
          <p className="text-sm text-slate-600 dark:text-slate-400 print:text-black">{label}</p>
          <div className="mt-2 flex items-baseline gap-2">
            <p className="text-3xl font-bold text-slate-900 dark:text-white print:text-black">{value}</p>
            {unit && <p className="text-sm text-slate-600 dark:text-slate-400 print:text-black">{unit}</p>}
          </div>
          {trend && (
            <p
